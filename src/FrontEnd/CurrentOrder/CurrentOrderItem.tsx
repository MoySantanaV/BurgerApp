...
const CurrentOrderItem = ({ order }: any) => {
  ...
  const onCompleteOrder = (id: string) => {
    const productsOnCurrentOrder = productsOrdered
      .filter((ele: Product) => ele.count > 0)
      .map((product: Product) => product.name);
    const total = productsOrdered.reduce(
      (total: number, order: Product) => total + order.price * order.count,
      0
    );
